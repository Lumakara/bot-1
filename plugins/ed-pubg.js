...
addLimit(m.sender, global.isPrem(m.sender), isCreator);
    ...
    if (!text) return Reply(`❌ Masukkan teks!\nContoh: .${command} NamaPUBG`)

    try {
        m.reply("⏳ Sedang memproses...")

        let api = `https://api.botcahx.eu.org/api/ephoto/coverpubg?apikey=${global.apikeyalip}&text=${encodeURIComponent(text)}`

        await alip.sendMessage(m.chat, {
            image: { url: api },
            caption: `✅ *COVER PUBG EPHOTO*\nText: ${text}`
        }, { quoted: m })
