...
addLimit(m.sender, global.isPrem(m.sender), isCreator);
    ...
    try {
        ...
        let api = `https://api.botcahx.eu.org/api/ephoto/grafitti?apikey=${global.apikeyalip}&text=${encodeURIComponent(text)}`

        await alip.sendMessage(m.chat, {
            image: { url: api },
            caption: `🎨 *GRAFFITI EPHOTO*\nText: ${text}`
        }, { quoted: m })
